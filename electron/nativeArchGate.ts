...
(() => {
  ...
  // thing that has bricked a shipped build before. SILLAGE_SKIP_ARCH_GATE=1
  ...
  if (process.env.SILLAGE_SKIP_ARCH_GATE === '1') {
    try { console.warn('[nativeArch] SILLAGE_SKIP_ARCH_GATE=1 — boot arch gate DISABLED'); } catch { /* best-effort */ }
    return;
  }
   ...
   *
   *   1. Electron sets `process.resourcesPath` to `…/Sillage.app/Contents/Resources`
  ...
  function isPackagedBuild(): boolean {
    ...
    // Real packaged apps: resourcesPath ends in /Sillage.app/Contents/Resources.
  ...
  process.on('uncaughtException', (err: NodeJS.ErrnoException) => {
    ...
    try {
      ...
      dialog.showErrorBox(
        packaged
          ? 'Sillage was built for a different chip — please reinstall'
