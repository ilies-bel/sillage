 ...
 *      `release/mac-arm64/Sillage.app/Contents/Resources` AND the current
...
const SHIPPED_ARM64_RESOURCES = path.join(REPO_ROOT, 'release', 'mac-arm64', 'Sillage.app', 'Contents', 'Resources');
const SHIPPED_X64_RESOURCES = path.join(REPO_ROOT, 'release', 'mac', 'Sillage.app', 'Contents', 'Resources');
...
describe('nativeArch packaged resolution (v2.8.2 fix)', () => {
  ...
  test('Packaged fix command is end-user-actionable, not a developer shell command', () => {
    ...
    // No public release page to link to; the message must still name the action.
    assert.ok(msg.includes('reinstall'), 'must tell the user to reinstall');
  ...
  test('NEGATIVE: a real wrong-arch .node tree under app.asar.unpacked fires ok:false', {
    ...
    const tmpResources = mkdtempSync(path.join(os.tmpdir(), 'sillage-arch-test-'));
    try {
  ...
  test('Missing .node in the unpacked tree is silently skipped (not a mismatch)', () => {
    ...
    const tmpResources = mkdtempSync(path.join(os.tmpdir(), 'sillage-arch-test-empty-'));
    try {
