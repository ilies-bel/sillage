 ...
 *
 * Why .mjs (not .ts):
 *   the electron tree imports .mjs directly via the existing ESM-style imports
 *   (as `modules/capture/systemAudioHealthClassifier.mjs` is imported),
...
// ---------------------------------------------------------------------------
// Targets: every third-party native addon Electron loads at runtime.
// Keep this list in sync with scripts/rebuild-native-electron.js `MODULES`.
//
// `better-sqlite3` used to head this list and no longer does: the store is
// `node:sqlite` (CLAUDE.md), so nothing in `electron/` requires it. Verifying
// it was not merely wasted work — it was a Windows tax. A guarded module has
// to be rebuilt from source for every arch this app packs, and node-gyp on
// Windows means MSVC and Python on every developer's machine, for a binary
// that is never dlopened. HR-2 makes Windows the primary platform; a phantom
// dependency that only bites there is exactly the wrong shape.
// ---------------------------------------------------------------------------
...
const PACKAGED_REINSTALL_MESSAGE =
  'This copy of Sillage was built for a different chip than your Mac.\n' +
  'Please reinstall the build that matches your CPU:\n\n' +
