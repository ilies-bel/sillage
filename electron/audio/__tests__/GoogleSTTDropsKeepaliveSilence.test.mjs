...
// the SAME audio devices produced correct transcripts on Deepgram.
//
...
// keepalives. Deepgram endpoints cleanly on the zero frames; Google's
...
const DUMMY_KEY = path.join(os.tmpdir(), `sillage-stt-test-sa-${process.pid}.json`);
fs.writeFileSync(
  DUMMY_KEY,
  JSON.stringify({
    type: 'service_account',
    project_id: 'sillage-stt-test',
    private_key_id: 'test',
    // Not a real key — never used because no RPC is issued in these tests.
    private_key: '-----BEGIN PRIVATE KEY-----\nMIIBVAIBADAN\n-----END PRIVATE KEY-----\n',
    client_email: 'test@sillage-stt-test.iam.gserviceaccount.com',
    client_id: '0',
