...
export function getBoundedOnnxSessionOptions(): OnnxThreadBounds {
    return {
        intraOpNumThreads: readIntEnv('SILLAGE_ONNX_INTRA_OP_THREADS', 1),
        interOpNumThreads: readIntEnv('SILLAGE_ONNX_INTER_OP_THREADS', 1),
        ...
        enableCpuMemArena: readBoolEnv('SILLAGE_ONNX_ENABLE_CPU_MEM_ARENA', false),
        enableMemPattern: readBoolEnv('SILLAGE_ONNX_ENABLE_MEM_PATTERN', false),
    };
}
...
function readMaxConcurrent(): number {
    return readIntEnv('SILLAGE_ONNX_MAX_CONCURRENT_SESSIONS', 2);
}

function readMinFreeGB(): number {
    const raw = process.env.SILLAGE_ONNX_MIN_FREE_GB;
    if (!raw) return 2.0;
 ...
 *
 * Override for tests / incident tuning with SILLAGE_ONNX_AVAILABLE_MEM_GB
 * (a fixed value forces the gate deterministically).
 */
export function getAvailableMemoryGB(): number {
    const override = process.env.SILLAGE_ONNX_AVAILABLE_MEM_GB;
    if (override) {
 ...
 * system has at least `SILLAGE_ONNX_MIN_FREE_GB` (default 2.0 GB) of
