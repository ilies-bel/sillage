...
export interface StoredCredentials {
    ...
    // Free trial state
    trialToken?: string;   // server-issued signed token
...
export class CredentialsManager {
    ...
    public getDefaultModel(): string {
        ...
        // the same prompt (measured), and faster output streaming — which is
        // the interactive latency target. Full Flash / Pro remain
    ...
    public setSttProvider(provider: 'none' | 'google' | 'groq' | 'openai' | 'deepgram' | 'elevenlabs' | 'azure' | 'ibmwatson' | 'soniox' | 'local-whisper'): boolean {
    ...
    // reload — matching `setDeepseekApiKey`. The Remove button
     ...
     *   - Stable domain/version tag (`'sillage-credential-fallback-v1'`) so a
    ...
    private getFallbackKey(): Buffer {
        ...
        const materialParts = [
            'sillage-credential-fallback-v1', // stable domain/version tag
        ];
