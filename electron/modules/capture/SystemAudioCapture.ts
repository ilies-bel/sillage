/**
 * System-audio (loopback) capture — the far end of the call.
 *
 * Moved from `electron/audio/SystemAudioCapture.ts` **as-is** (ARCHITECTURE.md §3).
 * This is the reason the rebuild stayed in this repo: dual-channel capture on
 * Windows is the single hardest thing in the product (HR-2) and it already
 * works. The comments below are the original ones and they are load-bearing —
 * each documents a bug that was expensive to find. See
 * `docs/reference/capture-invariants.md`.
 *
 * One change, and only one: the native class is resolved lazily instead of at
 * module scope. The old module-level `loadNativeModule()` ran on the static
 * import chain, which pulled in Electron and made this file unimportable —
 * hence untestable — outside a packaged app.
 */
import { EventEmitter } from 'node:events'
import { loadNativeModule, type NativeAudioModule } from './nativeModule.ts'

let resolved: NativeAudioModule['SystemAudioCapture'] | null | undefined

/** Resolved on first `start()`, never at import. */
const rustClass = (): NativeAudioModule['SystemAudioCapture'] | null => {
    if (resolved === undefined) {
        const result = loadNativeModule()
        if (!result.ok) console.error(`[SystemAudioCapture] native module unavailable: ${result.reason}`)
        resolved = result.ok ? result.module.SystemAudioCapture : null
    }
    return resolved
}

/** Tests only — the module-level resolution is deliberately sticky otherwise. */
export const __resetNativeClass = (): void => {
    resolved = undefined
}
...
export class SystemAudioCapture extends EventEmitter {
    ...
    constructor(deviceId?: string | null) {
        ...
        // LAZY INIT: do not construct the native monitor here, and do not even
        // probe for the native class. Constructing opens a cpal stream, which on
        // macOS lights the orange menu-bar microphone-in-use indicator for the
        // lifetime of the process with no meeting running; probing loads the
        // binary, which needs Electron. Both are deferred to start().
        console.log(`[SystemAudioCapture] Initialized wrapper (lazy). Device ID: ${this.deviceId || 'default'}`);
    }
    ...
    public start(): void {
        ...
        const RustAudioCapture = rustClass()
        if (!RustAudioCapture) {
