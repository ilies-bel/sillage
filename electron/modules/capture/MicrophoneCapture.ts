/**
 * Microphone capture — the rep's own voice.
 *
 * Moved from `electron/audio/MicrophoneCapture.ts` **as-is** (ARCHITECTURE.md §3),
 * with the same single change as its sibling: the native class is resolved
 * lazily rather than at module scope, so this file can be imported — and
 * therefore tested — without Electron.
 *
 * The pre-warm machinery below looks elaborate for what it is. It is not:
 * every branch of it is a bug that shipped once. See
 * `docs/reference/capture-invariants.md`.
 */
import { EventEmitter } from 'node:events'
import { loadNativeModule, type NativeAudioModule } from './nativeModule.ts'

let resolved: NativeAudioModule['MicrophoneCapture'] | null | undefined

/** Resolved on first `start()`, never at import. */
const rustClass = (): NativeAudioModule['MicrophoneCapture'] | null => {
    if (resolved === undefined) {
        const result = loadNativeModule()
        if (!result.ok) console.error(`[MicrophoneCapture] native module unavailable: ${result.reason}`)
        resolved = result.ok ? result.module.MicrophoneCapture : null
    }
    return resolved
}
...
export class MicrophoneCapture extends EventEmitter {
    ...
    public start(): void {
        ...
        const RustMicCapture = rustClass()
        if (!RustMicCapture) {
