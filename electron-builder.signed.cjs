 ...
 *   - identity        : Developer ID Application (auto-discovered, or SILLAGE_SIGN_IDENTITY/CSC_NAME)
 ...
 *   Sillage executable). So we build ONLY the `zip` target with electron-builder
 ...
 *
 *   Notarization credentials use the macOS keychain profile `sillage-notary`
...
process.env.SILLAGE_PRODUCTION_SIGN = '1';
...
process.env.APPLE_KEYCHAIN_PROFILE = process.env.APPLE_KEYCHAIN_PROFILE || 'sillage-notary';
...
const signIdentity =
  process.env.SILLAGE_SIGN_IDENTITY || process.env.CSC_NAME || undefined;
