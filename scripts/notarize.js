 ...
 *   - No-op when SILLAGE_SKIP_NOTARIZE=1 is set (explicit escape hatch).
...
module.exports = async function notarizeHook(context) {
  ...
  if (process.env.SILLAGE_SKIP_NOTARIZE === '1') {
    console.log('[notarize] SILLAGE_SKIP_NOTARIZE=1 — skipping notarization.');
    return;
  }
