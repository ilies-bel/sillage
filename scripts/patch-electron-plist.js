...
if (!content.includes('NSScreenCaptureUsageDescription')) {
  content = content.replace(
    ...
    '<key>NSScreenCaptureUsageDescription</key>\n\t<string>Sillage needs Screen Recording permission to capture system audio for meeting transcription.</string>\n\t<key>NSMicrophoneUsageDescription</key>'
  );
  modified = true;
...
if (!content.includes('NSAudioCaptureUsageDescription')) {
  content = content.replace(
    '<key>NSMicrophoneUsageDescription</key>',
    '<key>NSAudioCaptureUsageDescription</key>\n\t<string>Sillage needs system audio access to transcribe meeting audio.</string>\n\t<key>NSMicrophoneUsageDescription</key>'
  );
  modified = true;
...
// The app then renames itself to "Sillage" (app.setName + CFBundleName),
...
// promotion at startup paints exactly one correctly-timed "Sillage" tile.
//
...
if (content.includes('This app needs access to the microphone')) {
  content = content.replace(
    ...
    '<string>Sillage needs microphone access to transcribe your voice during meetings.</string>'
  );
  modified = true;
