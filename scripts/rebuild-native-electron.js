 ...
 *
 * `--build-from-source` is passed **on macOS only**, and reason 1 is the whole
 * reason: Rosetta is what makes a downloaded prebuilt binary untrustworthy.
 * Windows has no such translation layer — `process.arch` is the truth there —
 * so compiling from source buys nothing and costs MSVC plus Python on every
 * developer's machine and every CI runner. HR-2 makes Windows the primary
 * platform; making the primary platform pay for the secondary one's quirk is
 * backwards.
 */
...
/*
 * `keytar`, and nothing else. `better-sqlite3` was here until the store moved
 * to `node:sqlite` (CLAUDE.md) and nothing in `electron/` required it any more;
 * rebuilding it was a node-gyp toolchain requirement on every platform in
 * exchange for a binary that is never loaded.
 *
 * Keep in sync with `electron/lib/nativeArch.mjs` TARGETS and
 * `scripts/rebuild-native-for-target.cjs` MODULES.
 */
const MODULES = ['keytar'];
...
function main() {
  ...
  const fromSource = os.platform() === 'darwin';
