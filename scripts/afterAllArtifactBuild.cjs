 ...
 *        "The signature of the binary is invalid" @ Sillage.app/Contents/MacOS/Sillage
 ...
 * (APPLE_KEYCHAIN_PROFILE, e.g. `sillage-notary`). No-op if none are present.
 */
...
const VOLNAME = 'Sillage';
const BACKGROUND = path.resolve(__dirname, '..', 'assets', 'dmg-background.png');
const VOLICON = path.resolve(__dirname, '..', 'assets', 'sillage.icns');
...
function resolveDeveloperIdIdentity() {
  if (process.env.SILLAGE_SIGN_IDENTITY) return process.env.SILLAGE_SIGN_IDENTITY;
...
function buildStyledDmg({ appPath, outDmg, identity }) {
  ...
  // exactly [Sillage.app, Applications-droplink] and nothing stray.
  const stage = fs.mkdtempSync(path.join(os.tmpdir(), 'sillage-dmg-'));
  ...
  if (process.env.SILLAGE_DMG_BACKGROUND === '1' && fs.existsSync(BACKGROUND)) {
