 ...
 * -----------------------
 * `keytar` ships a SINGLE compiled binary at a fixed
 ...
 * arm64, so the x64 (`Sillage.dmg`) pack would embed arm64 binaries and every
 ...
 * THE FIX
 * -------
 * Before EACH per-arch pack, rebuild keytar for THAT target arch, pinned to the
 * installed Electron version — from source on macOS, where Rosetta makes a
 * downloaded prebuild untrustworthy; from the vendor's prebuild elsewhere,
 * because `--arch` already selects the right one and compiling instead would
 * put MSVC on the critical path of the primary platform (HR-2).
 *
...
//
// `better-sqlite3` left this list when the store moved to `node:sqlite`: it is
// no longer required anywhere in `electron/`, so packing it per-arch was a
// node-gyp toolchain requirement — MSVC and Python on the Windows runner —
// bought for a binary nothing dlopens.
...
module.exports = async function beforePack(context) {
  ...
  const fromSource = process.platform === 'darwin';

  console.log(
    `[beforePack] Rebuilding [${MODULES.join(', ')}] for target arch=${archName}, Electron ${electronVersion}` +
    `${fromSource ? ' (from source)' : ' (vendor prebuild for that arch)'}…`
  );
