...
// `define_obj_type!` below expands to a transmute clippy objects to. It is
// cidre's code, not ours, and an `#[allow]` on the invocation does not reach the
// expansion's span — so the crate's blanket `#![deny(clippy::all)]` is narrowed
// here, in the one file that instantiates an Objective-C class, rather than
// loosened at the top of lib.rs.
