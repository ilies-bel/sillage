...
mod tests {
    ...
    /// TEMPORARY PROBE. Run with:
    ///   GATE_PROBE_WAV=../scratch/speech.wav cargo test --lib gate_probe -- --nocapture
    /// Expects mono 16-bit PCM at 16 kHz.
    #[test]
    fn gate_probe() {
        let Ok(path) = std::env::var("GATE_PROBE_WAV") else {
            println!("gate_probe: GATE_PROBE_WAV unset, skipping");
            return;
        };
        let bytes = std::fs::read(&path).expect("wav");
        // Walk the RIFF chunks to the `data` payload.
        let mut off = 12usize;
        while off + 8 <= bytes.len() {
            let id = &bytes[off..off + 4];
            let sz = u32::from_le_bytes([bytes[off + 4], bytes[off + 5], bytes[off + 6], bytes[off + 7]]) as usize;
            if id == b"data" {
                off += 8;
                break;
            }
        ...
        let pcm: Vec<i16> = bytes[off..]
            .chunks_exact(2)
            .map(|c| i16::from_le_bytes([c[0], c[1]]))
            .collect();

        for (name, cfg) in [
            ("for_microphone (rms + ML vad)", SilenceSuppressionConfig::for_microphone()),
            (
                "rms only (use_vad = false)",
                SilenceSuppressionConfig { use_vad: false, ..SilenceSuppressionConfig::for_microphone() },
            ),
        ] {
            let mut s = SilenceSuppressor::new(SilenceSuppressionConfig { native_sample_rate: 16000, ..cfg });
            let (mut sent, mut keep, mut drop) = (0u32, 0u32, 0u32);
            let mut above_rms = 0u32;
            let mut frames = 0u32;
            for frame in pcm.chunks_exact(320) {
                frames += 1;
                if calculate_rms(frame) >= s.adaptive_threshold() {
                    above_rms += 1;
                }
                match s.process(frame).0 {
                    FrameAction::Send(_) => sent += 1,
                    FrameAction::SendSilence => keep += 1,
                    FrameAction::Suppress => drop += 1,
                }
            }
            let pct = |n: u32| 100.0 * f64::from(n) / f64::from(frames);
            println!(
                "{name}: frames={frames} past_rms={above_rms} ({:.0}%) -> sent={sent} ({:.0}%) keepalive={keep} suppressed={drop}",
                pct(above_rms),
                pct(sent),
            );
        }
