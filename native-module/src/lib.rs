...
// The audio surface, and nothing else. `license`, `stealth_window`, `keyboard_tap`
// and `process_name` went with demolition 3/5 and 4/5 — stealth is an explicit
// non-goal (VISION.md §2) and this is not a consumer app.
...
/// this replaces it.
#[napi]
pub fn abi_probe() -> String {
    concat!("sillage-audio ", env!("CARGO_PKG_VERSION")).to_string()
}
...
///
/// All targets supported by Sillage (macOS x64/arm64, Windows x64, Linux x64)
...
impl SystemAudioCapture {
    ...
    pub fn start(
        ...
        self.capture_thread = Some(thread::spawn(move || {
            ...
            // The far end is also the microphone path's echo reference. It is
            // published from here — the one place in the process that has it —
            // and read back by MicrophoneCapture's DSP thread; see
            // echo_reference.rs for why that seam is a process global and not a
            // handle threaded through JS.
            //
            // Only at the canonical rate. When the resampler failed to build
            // and this stream is passing through at the hardware rate, there is
            // nothing the other side could do with these samples except cancel
            // against a reference running at the wrong speed.
            let publishes_reference = emitted_rate == echo_reference::REFERENCE_RATE;
            // The generation this thread owns. Handed back on the way out so a
            // restart that overlaps this thread's teardown cannot have its
            // reference torn down by the stream it replaced.
            let mut reference_generation = 0u64;
            if publishes_reference {
                reference_generation = echo_reference::global().open();
            } else {
                println!(
                    "[SystemAudioCapture] Not publishing an echo reference: emitting {}Hz, the reference bus is {}Hz.",
                    emitted_rate,
                    echo_reference::REFERENCE_RATE
                );
            }
            ...
            loop {
                ...
                if !raw_batch.is_empty() {
                    let published_from = frame_buffer.len();
                    ...
                    raw_batch.clear();
                    // Published here, before the suppressor below gets a look at
                    // it. A canceller fed a gated, discontinuous reference
                    // cannot hold adaptation — and a suppressed frame is exactly
                    // the frame whose echo is still on its way to the mic.
                    if publishes_reference {
                        echo_reference::global()
                            .publish(reference_generation, &frame_buffer[published_from..]);
                    }
                }
            ...
            emitter.flush(&tsfn);
            // The far end is gone, so the microphone stops cancelling against it
            // rather than against whatever this left in the ring.
            if publishes_reference {
                echo_reference::global().close(reference_generation);
            }
...
impl MicrophoneCapture {
    ...
    pub fn start(
        ...
        self.capture_thread = Some(thread::spawn(move || {
            ...
            // Acoustic echo cancellation, against the reference SystemAudioCapture
            // publishes. `None` when the pipeline is not at the canonical rate or
            // when the graph will not build: either way the microphone runs
            // uncancelled rather than not at all (DEC-26 — nothing downstream may
            // stop a meeting being recorded).
            let mut canceller = EchoCanceller::new(emitted_rate);
            ...
            loop {
                ...
                while frame_buffer.len() >= chunk_size {
                    ...
                    // Ahead of the suppressor, deliberately: a canceller fed a
                    // gated signal cannot hold adaptation, and the frames the
                    // gate drops are exactly the ones carrying the far end's
                    // echo. No-op when no far-end capture is publishing.
                    if let Some(ref mut aec) = canceller {
                        aec.process(&mut frame_scratch);
                    }
